...
void datarate_result(void* ctxt, int* arate, int* vrate, int* drate) {
  ...
  if (datarate) {
    ...
    if (arate) { *arate = (int)(datarate->audio_bytes * 1000000.0 / tickdiff); } // b/s
    if (vrate) { *vrate = (int)(datarate->video_bytes * 1000000.0 / tickdiff); } // b/s
    if (drate) { *drate = (int)((datarate->audio_bytes + datarate->video_bytes) * 1000000.0 / tickdiff); } // b/s
    datarate->tick_start += tickdiff / 2; // 这里除以2的目的是，每次都只取前一次一般的byte进行计算，这样bytes率就不是整体的，而是局部的
...
void datarate_audio_packet(void* ctxt, AVPacket* pkt) {
  DataRate* datarate = (DataRate*)ctxt;
  if (datarate) { datarate->audio_bytes += pkt->size; }
}

void datarate_video_packet(void* ctxt, AVPacket* pkt) {
  DataRate* datarate = (DataRate*)ctxt;
  if (datarate) { datarate->video_bytes += pkt->size; }
}
