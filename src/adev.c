...
static void* audio_render_thread_proc(void* param) {
  ...
  while (!(context->status & ADEV_CLOSE)) {
    ...
    if (!(context->status & ADEV_CLOSE)) {
      ...
      if (++context->head == context->bufnum) { c->head = 0; }
...
void* adev_create(int type, int bufnum, int buflen, CommonVars* cmnvars) {
  ...
  context = (AdevContext*)calloc(1, sizeof(AdevContext) + bufnum * sizeof(int64_t) + bufnum * sizeof(AudioBuf));
  ...
  context->p_wave_buf = (uint8_t*)env->GetByteArrayElements(context->audio_buffer, 0);
...
#define MODE_STREAM 1
  ...
  jobject at_obj = env->NewObject(jcls, context->jmid_at_init, 
      STREAM_MUSIC, ADEV_SAMPLE_RATE, CHANNEL_STEREO, ENCODING_PCM_16BIT,
      context->buflen * 2, MODE_STREAM);
...
void adev_destroy(void* ctxt) {
  ...
  env->ReleaseByteArrayElements(context->audio_buffer, (jbyte*)context->p_wave_buf, 0);
...
void adev_write(void* ctxt, uint8_t* buf, int len, int64_t pts) {
  ...
  AdevContext* context = (AdevContext*)ctxt;
  pthread_mutex_lock(&context->lock);
  while (context->curnum == context->bufnum && (context->status & ADEV_CLOSE) == 0) {
    pthread_cond_wait(&context->cond, &context->lock);
  }

  if (context->curnum < context->bufnum) {
    memcpy(context->p_wave_hdr[context->tail].data, buf, MIN(context->p_wave_hdr[context->tail].size, len));
    context->curnum++; context->ppts[context->tail] = pts; 
    if (++context->tail == context->bufnum) { context->tail = 0; }
    pthread_cond_signal(&context->cond);
  }
  pthread_mutex_unlock(&context->lock);
#endif
